...
fun BaseDownLoadFragment.openDownLoadFile(item: PreviewItem) {
    val file = File(requireContext().getExternalFilesDir(null)?.path + "/" + item.name)
...
//                                        if (item.size <= 0) item.size =bytesRead
                                        ...
                                        tvPb?.run {
//                                            val  total = if (item.size > 0) ("/ " + Tools.getSize(item.size.toDouble())) else ""
                                            val total = ""

                                            text =
                                                "下载进度：${Tools.getSize(bytesRead.toDouble())} $total"
                                        }
                                    }
                                }
    ...
    val retrofitDownloadApi by lazy {
        Retrofit.Builder()
            .client(SingleRetrofit.getDownloadBuilder(downInterceptor))
            .baseUrl("base")
